...
- [ ] TASK-0.13 — Evaluate Phase 0 foundation (evaluation recorded; all automated gates, Windows packaging, and launch confirmed. Open only on PERF-1: cold start measured ~5s against a 3s target, warm start unmeasured — needs a reference-device measurement, optimisation, or an approved threshold change. See INTEGRATION_EVAL.md)
